...
while lbnorm > conv_crit && iter < maxiter...
    ...
    EA = squeeze(my_pagefun(@mtimes,Sigma_A,permute(b_mu_W_sum,[1,3,2])))';
    ...
    %% Update Tau
    trEAtSStA = zeros(V,1,B,'like',EA);
    for b = 1:B
        trEAtSStA(:,1,b) = squeeze(sum(sum(bsxfun(@times,ESSt(:,:,b),Sigma_A),2),1));
    end
    trEAtSStA = trEAtSStA+sum(bsxfun(@times,my_pagefun(@mtimes,EA,ESSt),EA),2);
%    % Faster, but requires more concurrent memory
%    trEAtSStA = permute(sum(sum(...
